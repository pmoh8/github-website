...
void mousePressed(){
  if(abs(mouseX-clickX1)<=abs(clickX2)&&abs(mouseY-clickY1)<=abs(clickY2)){
    dragMode = true;
    offsetX = mouseX-clickX1;
    offsetY = mouseY-clickY1;
  }
  else{
    clickX1 = mouseX;
    clickY1 = mouseY;
...
void mouseDragged(){
  if(dragMode){
    clickX1=mouseX-offsetX;
    clickY1=mouseY-offsetY;
  }
  else{
...
void drawSelectionBox(){
   for (int i = 0; i < numAttributes; i ++){
    ...
    if(clickY1 > chartOriginY && clickY1 < chartOriginY + chartLengthY){
      ...
      if(abs(clickX1-xCoord)<5&&!selectBoxActive){
          float drawY = clickY2;
...
ArrayList<Integer> findIntersectingLines(float x1, float y1, float xLength, float yLength){
...
void hoverInfo(){
  if (!axisSelect && !selectBoxActive){
    ...
    if(list.size()>0){
      ...
      rect(mouseX+15,mouseY-3*textAscent(),boxLength,7*textAscent());
      textAlign(LEFT);
      fill(color(0,0,0));
    ...
    if(list.size()>0){
      ...
      float boxLength = 1.1*max(textWidth(s1),textWidth(s2),textWidth(s3));
      fill(250,250,250,200);
      rect(mouseX+15,mouseY-3.5*textAscent(),boxLength,7*textAscent());
      textAlign(LEFT);
      fill(color(0,0,0));
      text(s1,mouseX+20,mouseY-20);
      text(s2,mouseX+20,mouseY);
